      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            ...
            { t: "note", variant: "note", title: "True maximum Health", text: "Some effects push you past the printed hearts (a Start-of-Game card, and the Flayed's errata). The Maximum Health marker can be removed from the board to reveal the final heart — that last heart is the character's true maximum Health." },
          ],
        },
      ],
    },
      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            ...
            { t: "h", text: "Round 0" },
            { t: "p", html: "A special round before Round I, triggered only if Equipment, Skills, or Monsters with the Round 0 icon are in play. It resolves left to right, like the other rounds. It's also when the Group can use <strong>Potions</strong>: discard a Potion to remove all Damage from one Beheaded." },
            { t: "h", text: "Range" },
      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            ...
            { t: "p", html: "Each Boss has at least 2 instances (the two card sides). When the first instance is defeated: the Turn ends immediately (ignore remaining Actions); remove all States/Shields from the Boss and the Beheaded; flip the Boss card; discard Active cards and draw a new Combat card; Combat restarts with the new instance from Round 0. Any excess Damage dealt to the defeated instance is ignored, and other board elements (Fire Strikes, Shurikens, etc.) remain." },
...

    {
      ...
      sections: [
        ...
        },
        {
          id: "errata",
          title: "Errata",
          summary: "Official corrections to printed components.",
          blocks: [
            { t: "note", variant: "warn", title: "Errata", text: "Official corrections from the publisher (FAQ v1.5)." },
            { t: "ul", items: [
              "<strong>The Flayed</strong> should not lose 1 Health the first time he gains a Survival upgrade — he only receives the Round 0 Shield bonus.",
              "<strong>Spider Runes</strong> in Encounter spaces should be ignored (a misprint from a previous version).",
              "The <strong>Level 3 Potion</strong> should be a During-Game card, like the earlier Potions — not Start of Game.",
              "<strong>Bobby's Feat cards</strong> should tell you to draw from the bottom of the Deck of Secrets.",
            ] },
          ],
        },
        {
          id: "faq",
          title: "FAQ",
          summary: "Official answers to common questions (FAQ v1.5).",
          blocks: [
            { t: "h", text: "Purge & decks" },
            { t: "faq", items: [
              { q: "How does the Purge action work?", a: "Slide a card into the slot in the tray to remove it from the game permanently. When no specific deck is named, you may choose a card from the Equipment deck or any Beheaded deck (including Serenade)." },
              { q: "Is there a minimum Combat Deck size?", a: "Yes — 6 for a Beheaded, 3 for Serenade." },
              { q: "After unlocking upgrade cards, must I keep using the starter cards?", a: "Yes. You can only remove starter (ST) cards with a Purge action." },
              { q: "Must I use all my Improved Combat cards every Run?", a: "Yes — you must use every Improved Combat card you've unlocked; you can't bench them in an unused deck. Only Purge removes cards." },
              { q: "When do we shuffle the discard pile into a new deck?", a: "When you need to draw and your draw pile is empty." },
            ] },
            { t: "h", text: "Biome setup & equipment" },
            { t: "faq", items: [
              { q: "Do we always use the same Equipment deck? What is a Biome's Blueprint deck?", a: "The Equipment deck (ST cards plus any Blueprints you've added) is always used when setting up a new Biome. Blueprint decks are Biome-specific (coded B1, B2, …); a Blueprint added to your Equipment deck immediately becomes Equipment and is no longer a Blueprint." },
              { q: "What if I must draw a Blueprint but the deck is empty?", a: "Nothing happens." },
              { q: "Can I voluntarily discard Equipment (e.g. to use a weapon that must be my only Equipment)?", a: "Yes, but only when you equip a new Equipment card, or during the Interbiome." },
              { q: "Equipment that scales with a Scroll track — what number do I use?", a: "The Level you've reached on that Scroll track, shown by the number just below your Scroll marker." },
              { q: "Can I use the same Equipment card multiple times in one Combat round?", a: "It depends on its class. Timed: once per round (and only 1 Timed per Combat card; in a 2-player game you may use 2 different Timed cards on different Combat cards). Always Active: as many times as it triggers. Skill: as many times as you have Skill actions." },
              { q: "Can a '/' (replace) Equipment be used if it would replace an empty action? And a '+' on an empty action?", a: "Yes to both." },
              { q: "Can I use the Old Wooden Shield multiple times in a round?", a: "No — it blocks only the first Damage, and only in Combat Round II." },
            ] },
            { t: "h", text: "Combat & states" },
            { t: "faq", items: [
              { q: "We remove all tokens after Combat — does that fully heal every Beheaded?", a: "No. Only Shields and States are removed; Damage stays." },
              { q: "Does Marked's +1 apply if the Damage is blocked?", a: "No — Marked applies only when a Damage token is actually placed. (If the Damage is Unblockable, the +1 is also Unblockable. Marked tokens stack.)" },
              { q: "Can I remove the Bleeding state before the round ends?", a: "Yes — enemies that inflict Bleeding have a Death Bonus that removes it." },
              { q: "What is the resolution order in Round 0?", a: "Left to right, the same as Rounds I–III." },
              { q: "Do Combat cards have to be played in full?", a: "Yes — Actions must be performed if you are able to." },
              { q: "If an Enemy performs multiple Actions in a round, are they separate and interruptible?", a: "Yes — each Action is separate and can be interrupted (e.g. a riposte that kills the Enemy before its second Action). BUT 'deal 1 Damage to 2 different opponents' and any ALL attack count as ONE Action with multiple targets, and are not interrupted." },
              { q: "Is an invisible Enemy invisible for the whole round?", a: "Yes, including during the Beheadeds' Actions. It can't be targeted by an ALL effect. Frozen only disables Invisible if applied before the round it turns invisible; once invisible, it can't be targeted at all." },
              { q: "What does the Wall/Hole token do?", a: "Some Equipment (e.g. the Rapier) and other elements behave differently when a Wall or Hole is present in Combat." },
              { q: "Are tokens limited by the number in the box?", a: "No — use substitutes if you run out." },
            ] },
            { t: "h", text: "Monsters" },
            { t: "faq", items: [
              { q: "The Kamikaze (Toxic Sewers) has a tombstone on its Round III row — what does that mean?", a: "It dies after its attack; you still collect its loot, if any." },
              { q: "If I Freeze a Kamikaze's Round III action, does it still die?", a: "No — the action was cancelled." },
            ] },
            { t: "h", text: "Bosses" },
            { t: "faq", items: [
              { q: "If a Beheaded dies during a Boss Combat, do we finish the whole Combat or just the Turn?", a: "The Run ends at the end of the current Turn (all of its Combat rounds)." },
              { q: "If we beat the Boss the same Turn a Beheaded dies, is it a victory?", a: "No — all Beheaded must survive the Combat." },
              { q: "When exactly do I flip the Boss card, and what is kept?", a: "The moment the Boss is defeated, the Turn ends (ignore remaining Actions). Flip the card, remove all tokens from the Boss and all States from the Beheaded, and restart from Round 0. Excess Damage on the first instance is ignored; special Boss cards and tokens (Fire Strikes, Shuriken) remain." },
              { q: "Does the Boss move toward the Beheaded at the end of a round?", a: "No — only Monsters move." },
              { q: "Do we keep playing after the Boss is beaten?", a: "No — Combat ends the moment the Boss dies." },
              { q: "How long does Freeze last on a Boss?", a: "Freeze tokens are removed at the end of the Turn." },
            ] },
            { t: "h", text: "2-player & solo" },
            { t: "faq", items: [
              { q: "(2-player) In what order are cards played, resolved, and discarded?", a: "The First Player's two cards are played and resolved in the order they choose at the start of Combat, and discarded in that same order." },
              { q: "(2-player) If a Freeze is applied to the First Player, which card is affected?", a: "The first-played card receives the token." },
              { q: "(2-player) If the First Player must discard a card, which one?", a: "The first-played card." },
              { q: "(Solo) When does Serenade reveal its card?", a: "After the player has chosen their Combat cards." },
              { q: "(Solo) Can Serenade be assigned State tokens?", a: "Yes — including Worm and Malaise tokens." },
              { q: "(Solo) On Serenade's second level, can the Vial heal Serenade?", a: "No — only the Beheaded." },
              { q: "(Solo) Can you assign Equipment cards to Serenade?", a: "No." },
            ] },
            { t: "h", text: "Death" },
            { t: "faq", items: [
              { q: "What happens to unspent Cells — can I keep them for the next Run?", a: "No — any unspent Cells are lost at the end of the Run." },
            ] },
          ],
        },
        {
          id: "faq-spoilers",
          title: "FAQ — Spoilers",
          summary: "Late-game tiles, Boss Cells, and final-boss questions.",
          blocks: [
            { t: "note", variant: "warn", title: "Spoilers ahead", text: "These answers reveal late-Biome tiles, Boss Cells, and final-boss mechanics. Skip this page if you haven't reached them yet." },
            { t: "h", text: "Biome tiles" },
            { t: "faq", items: [
              { q: "Does the symbol on tile B2-07 mean we gain the Vine Rune if we pass the check?", a: "Yes." },
              { q: "What do the icons on tile B4-10 mean?", a: "You can Purge an Equipment or Action card for the cost of 2 Gold Teeth." },
              { q: "What do the icons on tile B7-15 mean?", a: "You can buy a card from the Deck of Secrets for 5 Gold Teeth, but you must then Purge that tile." },
            ] },
            { t: "h", text: "Boss Cells" },
            { t: "faq", items: [
              { q: "With the Boss Cell active, the first Monster is placed behind the Group. If a Monster also has a special placement icon, which wins?", a: "The icon on the card takes priority; the Boss Cell replaces a Monster's normal starting position only." },
            ] },
            { t: "note", variant: "note", title: "Boss Cell card errata", text: "The Boss Cell card should read: \"Draw the bottom card from the Deck of Secrets the first time you do any of the following with the Boss Cell card in play: Beat the Angry Concierge; Beat the Angry Time Keeper; Beat the Hand of the King; Ring the 3 Bells.\"" },
            { t: "h", text: "Final-boss questions" },
            { t: "faq", items: [
              { q: "When the Time Keeper moves from behind the Group to the rightmost space, does she repeat her Action that round after the Beheaded Actions?", a: "Yes — follow the Golden Rule of Combat (left to right) as usual." },
              { q: "What does it mean to defeat a Hand of the King Sword card 'in the current Combat Round'?", a: "Ignore the reference to 'the current Combat Round' — you simply must defeat the Sword cards before you can deal any damage to the Hand of the King." },
              { q: "Does the Stamina Mutation apply to Serenade?", a: "No — Serenade doesn't have a Health track marker." },
            ] },
          ],
        },
        {
